...
from typing import Union, List, Callable, Tuple, Dict, Set, Any, Optional

# Set up a dedicated logger for this module
logger = logging.getLogger('simsig')

# Dynamically create the Signals enum from signals available on the current system
_AVAILABLE_SIGNALS: Dict[str, int] = {
    name: val for name, val in signal.__dict__.items()
...
class SimSig:
    ...
    def __init__(self):
        ...
        self._terminating_by_default: Set[Signals] = {
            s for s in Signals if s.name in [
                'SIGHUP', 'SIGINT', 'SIGQUIT', 'SIGILL', 'SIGABRT', 'SIGFPE',
                'SIGSEGV', 'SIGPIPE', 'SIGALRM', 'SIGTERM', 'SIGXCPU',
    ...
    def _normalize_signals(self, signals: Union[Signals, int, List[Union[Signals, int]]]) -> List[int]:
    ...
    def _create_fin_handler(self) -> Callable:
        ...
        if self._fin_handler is None:
            ...
            def handler(signum, frame):
                ...
                logger.warning(f"Received terminating signal {sig_name}. Initiating shutdown.")
    ...
    def set_handler(self, signals: Union[Signals, int, List[Union[Signals, int]]], reaction: Union[SigReaction, Callable]):
        ...
        for sig in signal_list:
            ...
            reaction_name = reaction.name if isinstance(reaction, SigReaction) else getattr(reaction, '__name__', str(reaction))
    ...
    def chain_handler(self, sig: Union[Signals, int], callback: Callable, order: str = 'before'):
        ...
        sig_name = Signals(sig).name
        logger.info(f"Chaining callback '{callback.__name__}' to {sig_name} handler (order: {order}).")
    ...
    def ignore_terminal_signals(self):
        ...
        terminal_signal_names = ['SIGHUP', 'SIGINT', 'SIGTSTP', 'SIGTTIN', 'SIGTTOU', 'SIGWINCH']
        signals_to_ignore = [Signals[name] for name in terminal_signal_names if name in Signals.__members__]
        if signals_to_ignore:
            logger.info(f"Ignoring terminal signals: {[s.name for s in signals_to_ignore]}.")
    ...
    def reset_to_defaults(self):
        ...
        for sig in Signals:
            try:
                ...
                logger.debug(f"Could not reset {sig.name}, signal is likely uncatchable.")
                continue

    @contextmanager
    def temp_handler(self, signals: Union[Signals, int, List[Union[Signals, int]]], reaction: Union[SigReaction, Callable]):
        ...
        logger.debug(f"Entering temp_handler context for signals {[Signals(s).name for s in signal_list]}.")
        try:
    ...
    def with_timeout(self, seconds: int):
        ...
        def _timeout_handler(signum, frame):
            raise SimSigTimeoutError(f"Code block did not complete in {seconds} seconds.")
    ...
    def block_signals(self, signals: Union[Signals, int, List[Union[Signals, int]]]):
        ...
        if not hasattr(signal, 'pthread_sigmask'):
            raise NotImplementedError("Signal masking (pthread_sigmask) is not supported on this OS.")
    ...
    def async_handler(self, signals: Union[Signals, int, List[Union[Signals, int]]], callback: Callable[..., Any]):
        """⚡ Registers a callback for use in an asyncio event loop."""
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError as e:
            raise RuntimeError("async_handler can only be called from within a running asyncio event loop.") from e
            
        signal_list = self._normalize_signals(signals)
        
        for sig in signal_list:
            logger.info(f"Registering async handler '{callback.__name__}' for {Signals(sig).name}.")
...
# 4. DEFAULT INSTANCE AND FUNCTIONAL API
# --------------------------------------
# For convenience, a default instance of SimSig is created.
# You can call these module-level functions directly without instantiating the class.

_default_instance = SimSig()

# --- Wrapper functions ---

def set_handler(signals: Union[Signals, int, List[Union[Signals, int]]], reaction: Union[SigReaction, Callable]):
...
def async_handler(signals: Union[Signals, int, List[Union[Signals, int]]], callback: Callable[..., Any]):
...
def temp_handler(signals: Union[Signals, int, List[Union[Signals, int]]], reaction: Union[SigReaction, Callable]):
