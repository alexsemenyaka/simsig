...
def test_chain_handler(clean_simsig):
    ...
    call_log = []
    def original_handler(s, f): call_log.append("original")
    def chained_handler(s, f): call_log.append("chained")
...
def test_set_handler_invalid_reaction_type(clean_simsig):
    ...
    with pytest.raises(TypeError, match="handler must be a SigReaction enum or a callable"):
...
def test_graceful_shutdown_invalid_callback_type(clean_simsig):
    ...
    with pytest.raises(TypeError, match="Provided callback must be a callable function"):
...
def test_chain_handler_invalid_order(clean_simsig):
    ...
    with pytest.raises(ValueError, match="Order must be 'before' or 'after'"):
        simsig.chain_handler(simsig.Signals.SIGUSR1, lambda s, f: None, order="sideways")
...
def test_async_handler_outside_loop(clean_simsig):
    ...
    with pytest.raises(RuntimeError, match="can only be called from within a running asyncio event loop"):
...
windows_only = pytest.mark.skipif(sys.platform != "win32", reason="This test is Windows-only.") ## Do we need this?
...
def test_with_timeout_restores_on_unrelated_exception(clean_simsig, mocker):
    ...
    mock_signal.assert_called_with(simsig.Signals.SIGALRM, original_handler) # Handler was restored
...
def test_temp_handler_suppresses_restore_error(clean_simsig, mocker):
    ...
    try:
        ...
        pytest.fail("temp_handler did not suppress the OSError during handler restoration.")
...
def test_reset_to_defaults_handles_errors(clean_simsig, mocker):
    ...
    mock_set_handler = mocker.patch.object(
        clean_simsig, 
        'set_handler', 
        side_effect=RuntimeError("Test-induced failure")
    )
...
def test_all_functional_api_wrappers_are_covered(clean_simsig, mocker):
    ...
    spy_chain.assert_called_once_with(simsig.Signals.SIGUSR1, dummy_func_with_args, order='before')
